...
module test_dftbplus_parser
  ...
  use dftbp_extlibs_hsd, only : hsd_table, hsd_error_t, hsd_get, hsd_set, new_table, &
      & hsd_load_string, HSD_STAT_OK, hsd_has_child, hsd_get_table
...
contains
  ...
  $:TEST("readMDInitTemp_kelvin", label="parser")
    ...
    call hsd_load_string("InitialTemperature [K] = 300.0", root, err)
  ...
  $:TEST("readMDInitTemp_hartree", label="parser")
    ...
    call hsd_load_string("InitialTemperature = 0.001", root, err)
    @:ASSERT(.not. allocated(err))
    pRoot => root
    call readMDInitTemp(pRoot, tempAtom, 0.0_dp)
  ...
  $:TEST("readMDInitTemp_minimum", label="parser")
    ...
    call hsd_load_string("InitialTemperature = 0.00001", root, err)
    @:ASSERT(.not. allocated(err))
    pRoot => root
  ...
  $:TEST("readParallel_defaults", label="parser")
    ...
    call hsd_load_string("Parallel {}", root, err)
    @:ASSERT(.not. allocated(err))
    call init(input)
    pRoot => root
  ...
  $:TEST("readParallel_absent", label="parser")
    ...
    call hsd_load_string("SomeOther = 1", root, err)
    @:ASSERT(.not. allocated(err))
    call init(input)
    pRoot => root
    call readParallel(pRoot, input)
