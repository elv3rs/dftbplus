...
contains
  ...
  !> necessary one.
  subroutine getCellTranslations(cellVec, rCellVec, latVec, recVec2p, cutoff, boundaryConds)
    ...
    type(TBoundaryConds), intent(in), optional :: boundaryConds
  ...
  subroutine getLatticePoints(latPoint, latVec, recVec2p, dist, boundaryConds, posExtension,&
    ...
    if (present(boundaryConds)) then
      call TLatPointIter_init(latPointGen, latVec, recVec2p, dist, negExtension, posExtension,&
          ...
          & dimensions=boundaryConds%nonContactDirs)
    else
      call TLatPointIter_init(latPointGen, latVec, recVec2p, dist, negExtension, posExtension,&
          & onlyInside, reduceByInversion, excludeOrigin=.true.)
    end if
  ...
  subroutine setNeighbourList(neigh, env, nNeighbour, iNeighbour, neighDist, cutOff, coord0,&
    ...
    !> Set of real space lattice vectors, R, over which there may be interacting atoms
