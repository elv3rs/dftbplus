...
module dftbp_capicallback
  ...
  !> See DftbPlusMatrixDescr in dftbplus.h
  type :: TCMatrixDescr
    !> Type of matrix: 0 - generic; 1 - hermitian or symmetric
    integer(c_int) :: matrix_type
    !> Type of storage: 0 - dense full; 1 - only lower triangle stored (upper for row-major i.e, C
    !! convention); 2 - only upper triangle stored (lower for row-major i.e., C convention).
